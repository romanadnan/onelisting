	...
	<div class="atbdp_login_form_shortcode">
		...
		<?php if ( directorist_is_email_verification_enabled() && ! empty( $_GET['send_verification_email'] ) ) : ?>
			<p class="directorist-alert directorist-alert-success"><span>
				<?php echo wp_kses( __( "Thank you for requesting a new verification email. Please check your inbox and verify to complete the registration.<br><br>If you still can't find it, please check your spam folder. And please contact if you are still having trouble.", 'onelisting' ), array( 'br' => array() ) ); ?>
			</span></p>
		<?php endif; ?>
		...
		if ( is_email( $user_email ) && ! empty( $key ) ) {
			...
			if ( ! $user ) { ?>
				<p class="directorist-alert directorist-alert-danger">
					<?php esc_html_e( 'Sorry! user not found', 'onelisting' ); ?>
				</p>
			<?php } else {
				...
				if ( is_wp_error( $is_valid_password_reset_key ) ) {
					...
					if ( ! empty( $_POST['directorist_reset_password'] ) && directorist_verify_nonce( 'directorist-reset-password-nonce', 'reset_password' ) ) :
						...
						if ( empty( $password_1 ) || empty( $password_2 ) ) : ?>
							<p class="atbd_reset_warning directorist-alert directorist-alert-danger"><?php echo esc_html__( 'Passwords cannot be empty.', 'onelisting' ); ?></p>
						<?php elseif ( $password_1 !== $password_2 ) : ?>
							<p class="atbd_reset_error directorist-alert directorist-alert-danger"><?php echo esc_html__( 'Passwords do not match!', 'onelisting' ); ?></p>
						<?php else :
							...
							<p class="atbd_reset_success directorist-alert directorist-alert-success"><?php echo wp_kses( sprintf(
								__( 'Password changed successfully. Please <a href="%s">click here to login</a>.', 'onelisting' ),
					...
					if ( ! empty( $_GET['confirm_mail'] ) ) {
						...
						<div class="directorist-alert directorist-alert-success">
							<?php echo wp_kses( sprintf(
								__( 'Email verification successful. Please <a href="%s">click here to login</a>.', 'onelisting' ),
								esc_url( ATBDP_Permalink::get_login_page_url() )
			...
			<div class="theme-modal-bottom">
				<?php if ( ! empty( $display_signup ) && $new_user_registration ) : ?>
					<p>
						...
						<a href="<?php echo esc_url( $reg_url ); ?>" data-bs-toggle="modal" data-bs-target="#theme-register-modal" data-bs-dismiss="modal"><?php echo esc_html( $reg_linktxt ); ?></a>
					</p>
				<?php endif; ?>
			</div>
			...
			if ( isset( $_POST['action'] ) && 'reset' === $_POST['action'] && directorist_verify_nonce() ) :
				...
				if ( empty( $email ) ) {
					$error = __( 'Email address cannot be empty.', 'onelisting' );
					...
					$subject   = esc_html( sprintf( __( '[%s] Reset Your Password', 'onelisting' ), get_option( 'blogname', 'display' )  ));
					$title     = esc_html__( 'Password Reset Request', 'onelisting' );
					...
					if ( $mail ) {
						...
						$error = __( 'Something went wrong, unable to send the password reset email. If the issue persists please contact with the site administrator.', 'onelisting' );
					}

				}

				if ( ! empty( $error ) ) {
					echo '<div class="message"><p class="error directorist-alert directorist-alert-danger">' . wp_kses( sprintf( __( '<strong>ERROR: </strong> %s', 'onelisting' ), esc_html( $error ) ), array( 'strong' => array() ) ) . '</p></div>';
				}
