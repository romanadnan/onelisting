...
<div class="add_listing_title atbd_success_mesage">
	...
	if ( ! empty( $_GET['registration_status'] ) && true == $_GET['registration_status'] ) {
		if ( empty( $display_password_reg ) ) {
			?>
			<p style="padding: 20px" class="alert-success"><?php directorist_icon( 'las la-check' ); ?><?php esc_html_e(' Go to your inbox or spam/junk and get your password.', 'onelisting'); ?>
				<?php
				$output = sprintf( __( 'Click %s to login.', 'onelisting' ), '<a href="' . ATBDP_Permalink::get_login_page_link() . '"><span style="color: red">' . __( 'Here', 'onelisting' ) . '</span></a>' );
				echo wp_kses_post( $output );
				?>
			</p>
		<?php } else { ?>
			<!--registration succeeded, so show notification -->
			<p style="padding: 20px" class="alert-success"><?php directorist_icon( 'las la-check' ); ?><?php esc_html_e(' Registration completed. Please check your email for confirmation.', 'onelisting'); ?>
				<?php
				$output = sprintf( __('Or click %s to login.', 'onelisting' ), '<a href="' . ATBDP_Permalink::get_login_page_link() . '"><span style="color: red">' . __( 'Here', 'onelisting' ) . '</span></a>' );
				echo wp_kses_post( $output );
				?>
			</p>
		<?php
...
<div class="directory_register_form_wrap">
		...
		<div class="theme-modal-wrap">
			...
			<?php if ( ! empty( get_directorist_option( 'display_user_type' ) ) ) {
				...
				<div class="atbd_user_type_area directory_regi_btn directorist-radio directorist-radio-circle directorist-mb-15">
					<input id="author_type" type="radio" name="user_type" value="author" <?php echo esc_attr( $author_checked ); ?>><label for="author_type" class="directorist-radio__label"><?php esc_html_e( 'I am an author', 'onelisting' ); ?>
				</div>
				<?php } ?>
				<?php if ( empty( $user_type ) || 'general' == $user_type ) { ?>
				<div class="atbd_user_type_area directory_regi_btn directorist-radio directorist-radio-circle directorist-mb-15">
					<input id="general_type" type="radio" name="user_type" value="general" <?php echo esc_attr( $general_checked ); ?>><label for="general_type" class="directorist-radio__label"><?php esc_html_e( 'I am a user', 'onelisting' ); ?>
				</div>
				<?php } ?>
			<?php } ?>
			...
			<div class="directory_regi_btn">
				<p><?php esc_html_e('Password will be e-mailed to you.','onelisting');?></p>
			</div>
			<?php } ?>
	...
	<div class="theme-modal-bottom">
		<?php if ( ! empty( $display_login ) ) { ?>
			<div class="directory_regi_btn">
				<p><?php echo esc_html( $login_text ); ?> <a href="<?php echo esc_url( $login_url ); ?>" data-bs-toggle="modal" data-bs-target="#theme-login-modal" data-bs-dismiss="modal"><?php echo esc_html( $log_linkingmsg ); ?></a></p>
			</div>
		<?php } ?>
	</div>
</div>